...
class Wpcf7_Color_Public {
    ...
    public function color_shortcode_handler($tag) {
        ...
        //if(wpcf7_is_posted() && wp_verify_nonce( $_REQUEST['_wpnonce'], 'redirect_nonce' ) && isset($_POST[$tag->name]) )
        if(wpcf7_is_posted() && wp_verify_nonce( $_REQUEST['_wpnonce'], 'redirect_nonce' ) && isset($_POST[$tag->name]) )
    ...
    public function color_validation( $result, $tag ) {
        ...
        $valueName = sanitize_text_field($_POST[$name]);
        $value = isset( $valueName )
            ? trim( wp_unslash( strtr( (string) $valueName, "\n", " " ) ) )
            : '';
