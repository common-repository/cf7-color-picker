...
class Wpcf7_Color_Admin {
    ...
    function wpcf7_color_load_admin_scripts( ) {
        ...
        if (isset($_GET['page']) && strpos($_GET['page'], 'wpcf7') !==false) {
            ...
            wp_register_script('my-script-handle', plugin_dir_url(__FILE__).'js/js-color-options.js', array('wp-color-picker'), WPCF7_COLOR_VERSION, false );
        }
    }
    ...
    public function tag_generator_color( $contact_form, $args = '' ) {
        ...
        <div class="insert-box">
            ...
            <div class="submitbox">
                <input type="button" class="button button-primary insert-tag" value="<?php echo esc_attr(__('Insert Tag', 'cf7-color-picker')); ?>" />
            </div>
            <br class="clear" />
        </div>
        <?php
        wp_nonce_field( 'cf7-color-nonce');
    }
}
